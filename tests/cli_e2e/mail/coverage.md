...
| ✓ | mail +draft-send | shortcut | mail_draft_send_workflow_test.go::TestMail_DraftSendWorkflowAsUser/send draft with shortcut as user; mail_draft_send_dryrun_test.go::TestMail_DraftSendDryRun; mail_draft_send_dryrun_test.go::TestMail_DraftSendDryRunScheduled | `--draft-id`; `--mailbox me`; `--yes`; dry-run repeated/comma-separated `--draft-id`; dry-run `--send-time` | sends a self-addressed draft through the batch shortcut and locks immediate plus scheduled dry-run request shape |
