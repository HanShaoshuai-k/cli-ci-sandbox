...
- 需要读取 Created By、Modified Time 等系统自动字段时，用 `lark-cli base +record-list --base-token <base_token> --table-id <table_id> --include-automatic-fields --limit 50`；仍然配合 `--field-id` 和 `--limit` 控制输出范围。
